...
3. 每天都可以学习和复习；自动任务的运行时段另行配置，不由星期几决定。
...
后续日期按日历日计算，落在周末也照常到期——想学的时候就该能学，日历上是星期几不构成阻拦。若同一个词的两个轮次落在同一天，则该词只展示一次，并用这次结果覆盖当天已经到期的轮次。
...
| `scheduled_on` | 计划日期 |
| `effective_due_on` | 实际到期日；目前等于 `scheduled_on`，历史数据中可能因周末顺延而不同 |
...
1. 每天是否固定为 6 个新词，还是允许根据复习量自动减少。
2. 每日允许显示多少个到期复习词。
3. 情景短文是否只使用新词，还是允许自然加入少量复习词。
4. 新词学习任务和到期复习任务分别在每天几点运行。
